...
function showEmployeeGalleryCard(i){
	...
	userHTML.className = 'card';
	document.getElementById('gallery').appendChild(userHTML);
	employeeFirstNames.push(employeeJSONdata[i].name.first);
}
...
function createEmployeeModalCard(i){
	...
	modalHTML += '<h3 id= ' + employeeJSONdata[i].name.first + 'class="modal-name cap">' + employeeJSONdata[i].name.first + ' ' + employeeJSONdata[i].name.last + '</h3>';
...
//This functon showsthe modal card  and handles the left, right and x buttons when clicked
function modalCardUI(i) {
	document.getElementById(employeeJSONdata[i].name.first).style.display = 'block';
...
function makeXMLHTTPUserRequest(i){
	...
	galleryXHR.onreadystatechange = function() {
		if(galleryXHR.readyState === XMLHttpRequest.DONE) {
			...
			//The employee modal card is opened when it is clicked
			document.getElementById(employeeJSONdata[i].name.first).addEventListener('click',()=>{
				modalCardUI(i);
			});
		}
	}
